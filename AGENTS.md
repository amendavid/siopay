...
- **Piège PostgREST : `count` est toujours `null` sur les mutations.** Sur une opération `.update()`, `.insert()` ou `.delete()`, l'option `{ count: 'exact', head: true }` passée à `.select()` ne peuple jamais `count` — la valeur revient systématiquement `null` (PostgREST ne renvoie le count que pour les requêtes GET, pas pour les mutations PATCH/POST/DELETE). **Conséquence :** un pattern comme `const { count } = await db.from('t').update(…).select('id', { count: 'exact', head: true }); return count > 0` est toujours faux, même quand la mutation a bien touché des lignes. **Pattern correct :** utiliser `.select('id')` sans `count` ni `head`, et vérifier `data.length > 0`. Ce bug a été découvert en S3 sur `writeTransactionStatus` — le CAS semblait toujours rater contre la vraie base alors que les tests unitaires mockés passaient.
